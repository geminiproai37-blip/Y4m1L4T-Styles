    ...
    if (item && item.poster_path) {
      ...
      slide.addEventListener("click", (event) => {
        if (!event.target.closest(".hero-button")) {
          const mediaType = item.media_type || 'tv'; // Default to tv if not specified
          const customUrl = `#${mediaType === 'tv' ? 'series' : 'movie'}/${item.id}`;
          window.location.hash = customUrl.substring(1); // Navigate using hash
        }
      });
  ...
  items.forEach((item) => {
    if (item && (item.poster_path || item.backdrop_path)) {
      ...
      contentCard.addEventListener("click", () => {
        // Prioritize custom URL from item data, fallback to default URL
        const finalUrl = item.customUrl || "http://localhost:3000/pagina-especifica";
...
async function handleNavigation() {
  ...
  } else if (hash.startsWith("go:")) {
    const mediaId = parseInt(hash.substring(3)); // Extract ID after "go:"
    const mediaItem = window.allMediaDetails.find(item => item.id === mediaId);
    if (mediaItem) {
      if (mediaItem.type === "tv") {
        await initMediaDetail(mediaId, "tv", null, null);
